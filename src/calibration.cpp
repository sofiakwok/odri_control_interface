      ...
      t_step_end_(0.)
{
    ...
    for (int i = 0; i < n_; i++)
    {
        if (search_methods_[i] == AUTO)
        {
            ...
            {
                std::cout << "positive search method" << std::endl;
                search_methods_[i] = POSITIVE;
            }
            else if (position_offsets[i] < (-M_PI / 2.) / gear_ratios_[i])
            {
                std::cout << "negative search method" << std::endl;
                search_methods_[i] = NEGATIVE;
            }
            else
            {
                std::cout << "alternative search method" << std::endl;
...
bool JointCalibrator::RunAndGoTo(VectorXd const& target_positions)
{
    ...
    {
        //std::cout << "calib state SEARCHING" << std::endl;
    ...
    {
        std::cout << "calib state GOTO" << std::endl;
...
{
    //std::cout << search_methods_[i] << std::endl;
    double des_pos = 0.;
    double des_vel = 0.;
